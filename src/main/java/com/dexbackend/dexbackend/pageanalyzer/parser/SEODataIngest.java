...
public class SEODataIngest implements DataAudits {
	...
	/** The logger. */
	private static Logger LOGGER=Logger.getLogger(SEODataIngest.class);
	...
	public void ingestData(String fileName, String buildID, String pageTitle, String applicationName) throws Exception 
	{
		try 
		{
			...
			if (!node.has(RUNTIME_ERROR)) 
			{
						...
						else 
						{
							...
							{		
								...
								if(jsonNodeOfDetails.isEmpty())
								{
									...
									LOGGER.fatal("Details of fail audit '"+categories.id+"' are not available");
									SEOFailedAudits seoFailedAudit = new SEOFailedAudits();
									seoFailedAudit.setRequestedUrl(isNull((JsonNode)node.path(REQUESTED_URL)));
									seoFailedAudit.setFetchTime(isNull((JsonNode)node.path(FETCH_TIME)));
									seoFailedAudit.setBuildId(buildID);
				...
				{	
				elasticSearchUtils.createIndex(ANALYZERSEOPASSAUDITS);
				elasticSearchUtils.ingestSEOPassAuditsToIndex(ANALYZERSEOPASSAUDITS, seoPassedAuditLists);
				}
				if(!seoFailedAuditLists.isEmpty())
				{
				elasticSearchUtils.createIndex(ANALYZERSEOFAILAUDITS);
				elasticSearchUtils.ingestSEOFailuresToIndex(ANALYZERSEOFAILAUDITS, seoFailedAuditLists);
				}
