...
public class ConfigSettingsDataIngest implements DataAudits{
	...
	private static final Gauge analyzerPerfConfigSettingGauge = Gauge.build().name(ANALYZERPERFCONFIGSETTING).help("exported from JSON data.")
			.labelNames(prometheusUtil.getDeclaredFields(ConfigSettings.class))
			.register();

	
	public static Gauge getAnalyzerPerfConfigSettingGauge() {
        return analyzerPerfConfigSettingGauge;
    }
	
	/** The logger. */
	private static Logger LOGGER=LoggerFactory.getLogger(ConfigSettingsDataIngest.class);
	...
	public void ingestData(String fileName, String buildID, String pageTitle, String applicationName, String userId)
		...
		if (configsettings != null) {
			LOGGER.info("Started - Ingesting analyzerperfconfigsetting to prometheus");
			prometheusUtil.ingestAuditsToPrometheus(getAnalyzerPerfConfigSettingGauge(), configsettings, prometheusUtil.getDeclaredFields(ConfigSettings.class), userId);
			LOGGER.info("Completed - Ingesting analyzerperfconfigsetting to prometheus");
		}
	}
