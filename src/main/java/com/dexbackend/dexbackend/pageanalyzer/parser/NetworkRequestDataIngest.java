...
public class NetworkRequestDataIngest implements DataAudits {
	...
	/** The logger. */
	private static Logger LOGGER=Logger.getLogger(NetworkRequestDataIngest.class); 
	...
	public void ingestData(final String fileName, final String buildID, final String pageTitle,
		...
		try {
			...
			ArrayList<NetworkRequestItems> tm = new ArrayList<NetworkRequestItems>();
			...
			for (Map<Object, Object> map : sm) {
				...
				tm.add(networkRequestItems);
			}
	
			elasticSearchUtils.createIndex(ANALYZERPERFNETWORKREQUEST);
			elasticSearchUtils.ingestNetworkRequestItemsToIndex(ANALYZERPERFNETWORKREQUEST, tm);
