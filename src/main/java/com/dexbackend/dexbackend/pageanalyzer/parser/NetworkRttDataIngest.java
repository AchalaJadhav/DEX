...
public class NetworkRttDataIngest implements DataAudits {
	...
	/** The logger. */
	private static Logger LOGGER=Logger.getLogger(NetworkRttDataIngest.class); 
	...
	public void ingestData(final String fileName, final String buildID, final String pageTitle,
		...
		try {
			...
			ArrayList<NetworkRttItems> tm = new ArrayList<NetworkRttItems>();
			...
			for (Map<Object, Object> map : sm) {
				...
				tm.add(networkRttItems);
			}
			elasticSearchUtils.createIndex(ANALYZERPERFNETWORKRTT);
			elasticSearchUtils.ingestNetworkRttItemsToIndex(ANALYZERPERFNETWORKRTT, tm);
