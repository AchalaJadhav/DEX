...
public class PerformanceDataIngest implements DataAudits {
	...
	/** The logger. */
	private static Logger LOGGER=Logger.getLogger(PerformanceDataIngest.class);
...
//								performanceCategoryList.add(performanceCategory);
				...
				{
					elasticSearchUtils.createIndex(ANALYZERPERFOPPO);
					elasticSearchUtils.ingestOppotunityItemsToIndex(ANALYZERPERFOPPO, opportunityItemsList);	
				}
				
				if(!performanceCategoryList.isEmpty()) {
				elasticSearchUtils.createIndex(ANALYZERPERFMETRICS);
				elasticSearchUtils.ingestMetricsItemsToIndex(ANALYZERPERFMETRICS, performanceCategoryList);
				}

				if(!performancePassedAuditsList.isEmpty()) {
				elasticSearchUtils.createIndex(ANALYZERPERFPASSAUDITS);
				elasticSearchUtils.ingestPerformancePassedAuditsItemsToIndex(ANALYZERPERFPASSAUDITS,
						performancePassedAuditsList);
				}
