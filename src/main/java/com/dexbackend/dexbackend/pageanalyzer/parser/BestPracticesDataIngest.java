...
public class BestPracticesDataIngest implements DataAudits {
	...
	/** The Constant for inspector-issues url. */
	...
	/** The logger. */
	private static Logger LOGGER=Logger.getLogger(BestPracticesDataIngest.class);
	
	@Override
	public void ingestData(String fileName, String buildID, String pageTitle, String applicationName) throws Exception {
		try {
			Thread.sleep(10000);
			...
			if (!node.has(RUNTIME_ERROR)) {
					...
					if (!categories.getGroup().isEmpty()) {
						...
						String title = auditNode.get(TITLE3).asText();
							...
							if (score < 1.0 ||  auditNode.get(SCORE3) == null) {	
										...
										bestPracticeItemDetails.setBestpracticesHeadingItemsMap(bestPracticesHeadingKeyItemsMap); 
										...
										ArrayList<Map<Object, Object>> sm = bestPracticesFailedAudits.getBestPracticeItemDetails().getBestpracticesHeadingItemsMap();
										...
										for (Map<Object, Object> map : sm ) {
											...
											case "external-anchors-use-rel-noopener":{
												//specific items
												bestPracticeItems.setPath(isNull((JsonNode)childNode.findPath(NODE).get(PATH)));

												break;
											}
											case "no-vulnerable-libraries":
											{
												bestPracticeItems.setHightestSeverity(isNull((JsonNode) map.get(HIGHTESTSEVERITY)));
												bestPracticeItems.setVulnCount(isNull((JsonNode) map.get(VULNCOUNT)));
												bestPracticeItems.setDectectedLibText(isNull((JsonNode)childNode.findPath(DETECTEDLIB).get(TEXT)));
												bestPracticeItems.setDectectedLibUrl(isNull((JsonNode)childNode.findPath(DETECTEDLIB).get(URL)));
											...
											case "image-size-responsive":{
												...
												bestPracticeItems.setDisplayedSize(isNull((JsonNode) map.get(TS_DISPLAYEDSIZE)));
											...
											case "errors-in-console":{
												...
												bestPracticeItems.setGUrl(isNull((JsonNode)childNode.findPath(SOURCELOCATION).get(G_URL)));
												break;
											}
											...
											case "inspector-issues":{
												...
												//specific items
												bestPracticeItems.setIUrl(isNull((JsonNode)childNode.findPath(SUBITEMS).findPath(ITEMS).get(IURL)));
												break;
											}
											case "no-unload-listeners":
											{
												bestPracticeItems.setNoType(isNull((JsonNode)childNode.findPath(SOURCE).get(TYPE)));
												bestPracticeItems.setValue(isNull((JsonNode)childNode.findPath(SOURCE).get(VALUE)));
												bestPracticeItems.setNoUrl(isNull((JsonNode)childNode.findPath(SOURCE).get(URL)));
											...
											case "image-aspect-ratio":
											{
												...
												bestPracticeItems.setDisplayedAspectRatio(isNull((JsonNode) map.get(DISPLAYEDRATIO)));
				...
				elasticSearchUtils.createIndex(BPPASSAUDITS);
				elasticSearchUtils.ingestBestPracticesPassAuditsToIndex(BPPASSAUDITS, bestPracticesPassedAuditLists);
				
		}
				if(!bestPracticeFailedAuditsList.isEmpty()) {
	
				elasticSearchUtils.createIndex(BPFAILAUDITS);
				elasticSearchUtils.ingestBestPracticesFailuresToIndex(BPFAILAUDITS, bestPracticeFailedAuditsList);
				}
			}
