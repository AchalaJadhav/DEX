...
public class NetworkServerLatencyDataIngest implements DataAudits {
	...
	/** The logger. */
	private static Logger LOGGER=LoggerFactory.getLogger(NetworkServerLatencyDataIngest.class); 
	
	private static final Gauge analyzerPerfNetworkLatencyGauge = Gauge.build().name(ANALYZERPERFNETWORKLATENCY).help("exported from JSON data.")
			.labelNames(prometheusUtil.getDeclaredFields(NetworkRequestLatencyItems.class))
			.register();
	
	public static Gauge getAnalyzerPerfNetworkLatencyGauge() {
        return analyzerPerfNetworkLatencyGauge;
    }
	...
	public void ingestData(final String fileName, final String buildID, final String pageTitle,
		...
		ArrayList<NetworkRequestLatencyItems> networkRequestLatencyItemsList = new ArrayList<NetworkRequestLatencyItems>();
		...
		for (Map<Object, Object> map : sm) {
			...
			networkRequestLatencyItemsList.add(networkRequestLatencyItems);
		}
		
		if (!networkRequestLatencyItemsList.isEmpty()) {
			LOGGER.info("Started - Ingesting analyzerperfnetworklatency to prometheus");
			for (NetworkRequestLatencyItems networkRequestItems : networkRequestLatencyItemsList) {
				prometheusUtil.ingestAuditsToPrometheus(getAnalyzerPerfNetworkLatencyGauge(), networkRequestItems, prometheusUtil.getDeclaredFields(NetworkRequestLatencyItems.class), userId);
			}
			LOGGER.info("Completed - Ingesting analyzerperfnetworklatency to prometheus");
		}
